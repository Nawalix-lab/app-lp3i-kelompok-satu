...
export default function BarangMasukScreen() {
  ...
  // =======================
  // FIX DI SINI → AMBIL USER DENGAN AWAIT
  // =======================
  useEffect(() => {
    fetchProducts();
  }, []);
  ...
  const fetchProducts = async () => {
    const { data: userData } = await supabase.auth.getUser();  // ← WAJIB await
    ...
    const { data, error } = await supabase
      .from("products")
      .select("*")
      .eq("user_id", userData.user.id)   // ← USER ID BENAR
      .order("name");
  ...
  const handleBarCodeScanned = ({ data }: { data: string }) => {
    ...
    if (foundProduct) {
      ...
      alert(`Barang dengan barcode ${data} tidak ditemukan.`);
    }
  };
  ...
  const handleSubmit = async () => {
    if (!selectedProduct) return alert("Pilih barang terlebih dahulu.");
      ...
      <ScrollView className="flex-1 px-5 pt-6" contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View className="flex-row gap-2">
            ...
            >
              <View className="flex-1 mr-2">
                <Text
                  className={`text-base ${selectedProduct ? "text-gray-900 font-semibold" : "text-gray-400"}`}
                  numberOfLines={1}
                >
        ...
        <View className="mb-6">
          ...
          <View className="flex-row items-center border border-gray-300 rounded-xl px-4 bg-white">
            ...
            <TextInput
              className="flex-1 py-4 ml-3 text-lg font-bold"
              placeholder="0"
        ...
        <TouchableOpacity
          ...
          className={`rounded-xl py-4 flex-row justify-center items-center ${
            loadingSubmit ? "bg-gray-400" : "bg-blue-600"
          }`}
        >
        ...
        <View className="flex-1 bg-black/50 justify-center items-center px-6">
          <View className="bg-white w-full rounded-3xl p-6 items-center shadow-xl">
            ...
            <TouchableOpacity
              ...
              className="bg-blue-600 w-full py-3 rounded-xl mt-4"
            >
