...
export default function BarangMasukScreen() {
  ...
  const handleBarCodeScanned = ({ data }: { data: string }) => {
    ...
    if (foundProduct) {
      ...
      Toast.show({
        type: 'success',
        text1: 'Produk Ditemukan',
        text2: foundProduct.name,
        position: 'top',
        visibilityTime: 2500,
      });
    } else {
      Toast.show({
        ...
        text2: `Barcode ${data} tidak ditemukan`,
        position: 'top',
        visibilityTime: 3000,
      });
    }
  ...
  const handleSubmit = async () => {
    if (!selectedProduct) {
      Toast.show({
        type: 'error',
        text1: 'Pilih barang terlebih dahulu',
        position: 'top',
        visibilityTime: 3000,
      });
      return;
    ...
    try {
      ...
      if (logError) {
        Toast.show({
          type: 'error',
          text1: 'Gagal mencatat stok',
          text2: logError.message,
          position: 'top',
          visibilityTime: 3000,
        });
        return;
