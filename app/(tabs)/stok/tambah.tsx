...
export default function TambahProdukScreen() {
  ...
  const [minStock, setMinStock] = useState("10"); // state baru untuk minimal stok
    ...
    if (!permission?.granted) {
      ...
      if (!granted) {
        Toast.show({
          ...
          text2: 'Aktifkan izin kamera untuk scan barcode',
          position: "top",
          visibilityTime: 3000,
        });
        return;
  ...
  async function handleSave() {
    const minStockVal = parseInt(minStock) || 0;
    if (!name.trim()) {
      Toast.show({
        type: 'error',
        text1: 'Nama barang wajib diisi!',
        position: 'top',
        visibilityTime: 3000,
      });
      setLoading(false);
    ...
    try {
      ...
      if (!price.trim() || parseInt(price) <= 0) {
        console.error("Harga harus diisi dan lebih dari 0");
        Toast.show({
          type: "error",
          text1: "Harga wajib diisi!",
          text2: "Masukkan harga yang valid (> 0).",
          position: "top",
          visibilityTime: 3000,
        });
        setLoading(false);
      ...
      if (barcode.trim()) {
        ...
        if (minStockVal < 0) {
          Toast.show({
            type: 'error',
            text1: 'Minimal stok tidak boleh negatif!',
            text2: 'Masukkan minimal stok yang valid (> 0).',
            position: 'top',
            visibilityTime: 3000,
          });
          setLoading(false);
      ...
      Toast.show({
        type: 'error',
        text1: 'Gagal menyimpan data produk',
        text2: error.message,
        position: 'top',
        visibilityTime: 3000,
      });
    ...
    <KeyboardAvoidingView style={{ flex: 1 }}
      ...
      <View className="flex-1 bg-white dark:bg-gray-900">
        ...
        <ScrollView className="flex-1 px-5 pt-6" contentContainerStyle={{ paddingBottom: 40 }}>
              ...
              <View className="flex-row flex-wrap gap-2">
                {categoriesList.map((cat, index) => (
                  <TouchableOpacity
                    ...
                    className={`px-3 py-2.5 rounded-xl border items-center justify-center ${category === cat
                  ...
                  >
                    <Text
                      ...
                      className={`text-xs font-semibold ${category === cat ? 'text-white' : 'text-gray-600 dark:text-gray-300'
                        }`}
                    >
                      {cat}
                    </Text>
          ...
          <View className="flex-row gap-4 mb-4">
            <View className="flex-1">
              <Text className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Harga Jual <Text className="text-red-500">*</Text></Text>
              <TextInput
                className="border border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-base bg-white dark:bg-gray-800 dark:text-white"
                placeholder="0"
                placeholderTextColor={colorScheme === 'dark' ? '#9CA3AF' : '#9CA3AF'}
          ...
          {/* Minimal Stok */}
          <View className="mb-8 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
            <Text className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Minimal Stok</Text>
            <TextInput
              className="bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-base font-semibold dark:text-white"
              placeholder="0"
              placeholderTextColor={colorScheme === 'dark' ? '#9CA3AF' : '#9CA3AF'}
            ...
            <Text className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Minimal stok yang diperbolehkan sebelum produk dianggap habis. Default: 10.
            </Text>
          </View>
